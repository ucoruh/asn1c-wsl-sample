...
INTEGER__dump(const asn_TYPE_descriptor_t *td, const INTEGER_t *st, asn_app_consume_bytes_f *cb, void *app_key, int plainOrXER) {
	...
	if(ret == 0) {
		...
		if(el) {
			if(plainOrXER == 0)
				return asn__format_to_callback(cb, app_key,"%"ASN_PRIdMAX " (%s)", value, el->enum_name);
			else
            ...
            return asn__format_to_callback(cb, app_key,(specs && specs->field_unsigned) ? "%"ASN_PRIuMAX : "%"ASN_PRIdMAX,value);
        }
	} else if(plainOrXER && specs && specs->strict_enumeration) {
		/*
